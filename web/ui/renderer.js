...
function applyDSPChain(buffer, settings, onProgress = null, logPrefix = '[DSP]') {
  ...
  const sourceReferenceBuffer = buffer;
  let aiProfile = {
    softClipDrive: 1.5,
  ...
  if (artifactSafeMode && !losslessTransparentMasterMode) {
    ...
    if (useSampleCrackleRepair) {
      ...
      renderedBuffer = applyVocalMidCrackleSuppressor(renderedBuffer, {
    ...
    const rescueAnalysis = !losslessArtifactSafeMode && artifactAmount >= 0.88
      ? analyzeAIGeneratedMastering(renderedBuffer)
      : null;
    ...
    if (needsMetallicToneCut) {
      ...
      const deEssed = applyDynamicSibilanceSuppressor(renderedBuffer, {
      ...
      renderedBuffer = deEssed.buffer;
      if (rescued.moves) {
        console.log(`${logPrefix} Metallic rescue moves:`, rescued.moves);
      }
      if (deEssed.moves) {
        console.log(`${logPrefix} Post-metallic dynamic de-ess moves:`, deEssed.moves);
      }
      ...
      renderedBuffer = deEssed.buffer;
      if (deEssed.moves) {
        console.log(`${logPrefix} Artifact-safe dynamic de-ess moves:`, deEssed.moves);
      }
    }
  }
  ...
  if (!artifactSafeMode && settings.aiEnhance !== false) {
    ...
    renderedBuffer = deEssed.buffer;
    if (deEssed.moves) {
      console.log(`${logPrefix} Final dynamic de-ess moves:`, deEssed.moves);
    }
  }
  ...
  if (settings.aiEnhance !== false && losslessArtifactSafeMode) {
    ...
    renderedBuffer = sourceConstrained.buffer;
    if (sourceConstrained.moves && !sourceConstrained.moves.skipped) {
      console.log(`${logPrefix} Source-constrained sibilance repair moves:`, sourceConstrained.moves);
    }

    const vocalBuzz = applySourceConstrainedVocalBuzzRepair(renderedBuffer, sourceReferenceBuffer, {
    ...
    renderedBuffer = vocalBuzz.buffer;
    if (vocalBuzz.moves && !vocalBuzz.moves.skipped) {
      console.log(`${logPrefix} Source-constrained vocal buzz repair moves:`, vocalBuzz.moves);
    }

    const guarded = applyAddedSibilanceGuard(renderedBuffer, sourceReferenceBuffer, {
    ...
    renderedBuffer = guarded.buffer;
    if (guarded.moves && !guarded.moves.skipped) {
      console.log(`${logPrefix} Source-differential sibilance guard moves:`, guarded.moves);
    }

    const finalVocalBuzz = applySourceConstrainedVocalBuzzRepair(renderedBuffer, sourceReferenceBuffer, {
    ...
    renderedBuffer = finalVocalBuzz.buffer;
    if (finalVocalBuzz.moves && !finalVocalBuzz.moves.skipped) {
      console.log(`${logPrefix} Final source-constrained vocal buzz repair moves:`, finalVocalBuzz.moves);
    }
  }
...
export async function renderOffline(sourceBuffer, settings, onProgress, options = {}) {
  ...
  if (
    settings.aiEnhance !== false &&
    losslessArtifactSafeMode &&
    ...
    renderedBuffer = toneGuarded.buffer;
    if (toneGuarded.moves && !toneGuarded.moves.skipped) {
      console.log('[Offline Render] Source-differential tone guard moves:', toneGuarded.moves);
    }
    const sourceConstrained = applySourceConstrainedSibilanceRepair(renderedBuffer, sourceBuffer, {
      amount: 0.86,
    ...
    renderedBuffer = sourceConstrained.buffer;
    if (sourceConstrained.moves && !sourceConstrained.moves.skipped) {
      console.log('[Offline Render] Post-safety source-constrained sibilance repair moves:', sourceConstrained.moves);
    }
    const vocalBuzz = applySourceConstrainedVocalBuzzRepair(renderedBuffer, sourceBuffer, {
      amount: 0.78,
    ...
    renderedBuffer = vocalBuzz.buffer;
    if (vocalBuzz.moves && !vocalBuzz.moves.skipped) {
      console.log('[Offline Render] Post-safety source-constrained vocal buzz repair moves:', vocalBuzz.moves);
    }
    const guarded = applyAddedSibilanceGuard(renderedBuffer, sourceBuffer, {
      amount: 0.82,
      threshold: 0.018,
    ...
    renderedBuffer = guarded.buffer;
    if (guarded.moves && !guarded.moves.skipped) {
      console.log('[Offline Render] Post-safety source-differential sibilance guard moves:', guarded.moves);
    }
    const finalVocalBuzz = applySourceConstrainedVocalBuzzRepair(renderedBuffer, sourceBuffer, {
      amount: 0.76,
    ...
    renderedBuffer = finalVocalBuzz.buffer;
    if (finalVocalBuzz.moves && !finalVocalBuzz.moves.skipped) {
      console.log('[Offline Render] Final source-constrained vocal buzz repair moves:', finalVocalBuzz.moves);
    }
  }
